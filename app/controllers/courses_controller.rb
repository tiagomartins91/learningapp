class CoursesController < ApplicationController
  ...
  def index
    ...
    @pagy, @courses = pagy(@ransack_courses.result(distinct: true).includes(:user))
  end
  ...
  def destroy
    ...
    if @course.destroy
      ...
      redirect_to @course, alert: 'Course has enrollments. Can not be destroy!'
    end
  end
  ...
  private
    ...
    def course_params
      params.require(:course).permit(:title, :description, :short_description,
                                     :language, :level, :price, :published, :avatar)
    end
end
